...
public class Player {//user behavior
    ...
    public boolean move(int color,int whichPlane,int dice){
        ...
        int curPos = Game.getChessBoard().getAirplane(color).position[whichPlane];
        Game.getChessBoard().getAirplane(color).curPos[whichPlane] = curPos;
        int nextStep = curPos + dice;
        if(nextStep > 56) {
            nextStep = 56 - (nextStep - 56);
            Game.getChessBoard().setOverflow(true);
        }
        else if(nextStep == 56)
            nextStep = -2;
        else if(nextStep == 18) {
